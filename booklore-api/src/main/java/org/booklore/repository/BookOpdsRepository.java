...
public interface BookOpdsRepository extends JpaRepository<BookEntity, Long>, JpaSpecificationExecutor<BookEntity> {
    ...
    @Query("SELECT b.id FROM BookEntity b LEFT JOIN b.metadata m WHERE (b.deleted IS NULL OR b.deleted = false) ORDER BY "
		...
		+ "CASE WHEN :sortBy = 'PUBLISHED_ASC' THEN m.publishedDate END ASC, "
		+ "CASE WHEN :sortBy = 'PUBLISHED_DESC' THEN m.publishedDate END DESC, "
	...
	@Query("SELECT DISTINCT b.id from BookEntity b JOIN b.userBookProgress p JOIN p.user u JOIN b.metadata m WHERE u.id = :userId AND (b.deleted IS NULL OR b.deleted = false) "
		+ "AND p.readStatus = 'UNREAD' ORDER BY "
		+ "CASE WHEN :sortBy = 'TITLE_ASC' THEN m.title END ASC, "
		+ "CASE WHEN :sortBy = 'TITLE_DESC' THEN m.title END DESC, "
		+ "CASE WHEN :sortBy = 'AUTHOR_ASC' THEN (SELECT MIN(a.name) FROM m.authors a) END ASC, "
		+ "CASE WHEN :sortBy = 'AUTHOR_DESC' THEN (SELECT MIN(a.name) FROM m.authors a) END DESC, "
		...
		+ "CASE WHEN :sortBy = 'SERIES_DESC' THEN COALESCE(m.seriesNumber, 999999) END DESC, "
		+ "CASE WHEN :sortBy = 'PUBLISHED_ASC' THEN m.publishedDate END ASC, "
		+ "CASE WHEN :sortBy = 'PUBLISHED_ASC' THEN m.seriesName END ASC, "
		+ "CASE WHEN :sortBy = 'PUBLISHED_ASC' THEN COALESCE(m.seriesNumber, 999999) END ASC, "
		+ "CASE WHEN :sortBy = 'PUBLISHED_DESC' THEN m.publishedDate END DESC, "
		+ "CASE WHEN :sortBy = 'PUBLISHED_DESC' THEN m.seriesName END DESC, "
		+ "CASE WHEN :sortBy = 'PUBLISHED_DESC' THEN COALESCE(m.seriesNumber, 999999) END DESC, "
		+ "b.addedOn DESC")
	Page<Long> findUnreadBookIds(@Param("userId") Long userId, @Param("sortBy") String sortBy, Pageable pageable);
}